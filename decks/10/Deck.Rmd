...
```{r make_fun, echo=FALSE, include=FALSE}
f <- function(x, y) {x+y-y^2-x^2-x*y}
```
...
- $f'(0,y) = 1 - 2y$
- Stationary at $y = 1/2$
- $f(0, 1/2) = 1/4$; $f(0,0) = `r f(x=0,y=0)`$; $f(0,2) = `r f(x=0,y=2)`$
- This shows that $f(0,y)$ is maximized at $(0,1/2)$ with value $1/4$
- and minimized at $(0,2)$ with value $`r f(x=0,y=2)`$
...
- Thus, the optima must occur at the endpoints: $f(0,2) = `r f(x=0,y=2)`$, while $f(2,2) = `r f(x=2, y=2)`$.
